...
get_stats <- function(year, year2) {
  ...
  for (year in seq_along(years)) {
    ...
    passurl <- glue("https://www.pro-football-reference.com/years/", year, "/passing.htm")
    rushurl <- glue("https://www.pro-football-reference.com/years/", year, "/rushing.htm")
    receiveurl <- glue("https://www.pro-football-reference.com/years/", year, "/receiving.htm")
    fantasyurl <- glue("https://www.pro-football-reference.com/years/", year, "/receiving.htm")
      
      pass <- read_html(passurl) %>% 
        html_nodes("table") %>% 
        html_table()
  
      rush <- read_html(rushurl) %>% 
        html_nodes("table") %>% 
        html_table()
      
      receive <- read_html(receiveurl) %>% 
        html_nodes("table") %>% 
        html_table()
      
      fantasy <- read_html(fantasyurl) %>% 
        html_nodes("table") %>% 
        html_table()
  }
